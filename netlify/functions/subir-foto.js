  ...
  try {
    const { datos } = await mutarDatos(datosStore, CLAVE, (datosActuales) => {
      ...
      for (const categoria of datosActuales) {
        ...
        if (modelo) {
          rutaAnterior = modelo.imagen || null;
    ...
    // La foto vieja de esa variante ya no se usa: la borramos para no acumular
    // basura en el storage. Se hace despues de confirmar el guardado, y si falla
    // no afecta la respuesta (la foto nueva ya quedo asociada correctamente).
    if (rutaAnterior && rutaAnterior !== rutaPublica) {
      await borrarImagenSiExiste(imagenes, rutaAnterior);
    }
