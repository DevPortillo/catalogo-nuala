...
exports.handler = manejarErrores(async (event) => {
  ...
  if (!datos) {
    ...
    let escritura = null;
    try {
      escritura = await store.setJSON(CLAVE, datosIniciales, { onlyIfNew: true });
    } catch (error) {
      console.error('get-datos: fallo al sembrar datos iniciales', error);
    }
    const seGuardo = !!(escritura && typeof escritura === 'object' && escritura.modified === true);
    datos = seGuardo ? datosIniciales : (await store.get(CLAVE, { type: 'json' })) || datosIniciales;
  }
